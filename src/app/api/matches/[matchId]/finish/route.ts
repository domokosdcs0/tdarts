// app/api/matches/[matchId]/finish/route.ts
...
export async function PATCH(request: Request, { params }: { params: { matchId: string } }) {
  try {
    ...
    const { winnerId, player1LegsWon, player2LegsWon, stats } = await request.json();
    console.log("Received data:", { matchId, winnerId, player1LegsWon, player2LegsWon, stats });

    // Validáció
    if (!winnerId || player1LegsWon == null || player2LegsWon == null) {
    ...
    if (match.status !== "ongoing") {
      return NextResponse.json({ error: "A mérkőzés nem játszható állapotban van" }, { status: 400 });
    }

    // Debug: Log player1 and player2 IDs
    ...
    if (!group.standings || group.standings.length === 0) {
      ...
      // Ensure match players are included in standings
    ...
    if (player1Standing && player2Standing) {
      ...
      console.error("Standings not found for players:", {
        ...
        player1Id: match.player1._id.toString(),
        player2Id: match.player2._id.toString(),
        standings: group.standings.map((s: any) => s.playerId.toString()),
      });
      return NextResponse.json({ error: "Nem sikerült a csoport állás frissítése" }, { status: 500 });
    }

    // Standings rendezése: pontok (desc), leg különbség (desc), legek nyerve (desc)
    ...
    // Rank frissítése
    group.standings.forEach((s: any, index: number) => {
      s.rank = index + 1;
    });

    console.log("Updated standings:", group.standings.map((s: any) => ({
      playerId: s.playerId.toString(),
      points: s.points,
      legsWon: s.legsWon,
      legsLost: s.legsLost,
      legDifference: s.legDifference,
      rank: s.rank,
    })));
    ...
    // Tábla állapotának frissítése: idle → waiting
